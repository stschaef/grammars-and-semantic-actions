...
{- Syntax for strictly positive endofunctors on a model.
   ...
   `⟦ F ⟧ : (X → Grammar) → Grammar` interprets a code, and `map`
   gives its functorial action.

   Compared with `Grammar.Inductive.Functor` there are no `LiftG`s:
   a model has a single level of objects, so the codes and their
   interpretations live at that level and no lifting is needed.
-}
...
data Functor (X : Type ℓX) : Type (ℓ-max ℓ (ℓ-suc ℓX)) where
  k : Grammar → Functor X
  Var : X → Functor X
