...
{- `Grammars/Arith` run at all three answers: the same `step` at three
   `AnswerFunctor`s, which agree on every case -- the enumeration has exactly
   one element, the grammar being unambiguous.  The action is `semact-pure
   tt`: compared is whether and how many, not the tree (`Decidable/Arith`). -}
open import Cubical.Foundations.Prelude
open import Theory.Instances.Monoid.Combinator.Grammars.ArithGrammar
  using (Tok ; nm ; pl ; lb ; rb ; _≟T_ ; Exp ; Lang)

module Examples.Theory.Combinator.Arith where
...
-- at `ND` a `just` becomes a one-element enumeration (unambiguity)
...
-- stress: deep chains and nests, through the Dec pipeline
chain : ℕ → List Tok
chain zero = nm ∷ []
...
_ : parseDec (chain 32) ≡ M.just tt
_ = refl

_ : parseDec (nest 32) ≡ M.just tt
_ = refl

_ : parseDec (lb ∷ nest 32) ≡ M.nothing
_ = refl
