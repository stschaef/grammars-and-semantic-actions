...
{- One regex, compiled once (`Combinator/Syntax`'s `⟦_⟧P`), run at all three
   answers.  `amb` is `(a ∣ aa)*`: chops of `aⁿ` number `Fib (n+1)`. -}
open import Cubical.Foundations.Prelude

module Examples.Theory.Combinator.Regex where
...
-- `amb` is where they part: `Dec` and `Maybe` return one chop; `ND` the whole `Fib (n+1)` fan.
amb-count : passes
