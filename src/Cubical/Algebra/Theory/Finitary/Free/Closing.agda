...
open import Cubical.Algebra.Theory.Finitary
  using (SortedSig; SortedEqns; Tm; var; node; Ops; TmRec; arities; vars;
...
ℓClosing : (ℓS ℓ ℓ'' ℓv : Level) → Level
ℓClosing ℓS ℓ ℓ'' ℓv = ℓ-max (ℓ-max ℓS ℓ) (ℓ-max ℓ'' ℓv)

module _ {S : Type ℓS} {σ : SortedSig S ℓ}
  (σeq : SortedEqns σ ℓ'') where

  data FreeModel (V : Type ℓv) (vs : V → S)
    : S → Type (ℓClosing ℓS ℓ ℓ'' ℓv) where
    var : (v : V) → FreeModel V vs (vs v)
    node : (o : σ .ops)
      → ((a : arities σ o) → FreeModel V vs (σ .sortOf o a))
      → FreeModel V vs (σ .resultSort o)
    clo : (e : σeq .eqns) {s : S}
      → Tm σ (vars σeq e) (σeq .varSort e) s
      → ((w : vars σeq e) → FreeModel V vs (σeq .varSort e w))
      → FreeModel V vs s
    cloVar : (e : σeq .eqns) (w : vars σeq e)
      (ρ : (w' : vars σeq e) → FreeModel V vs (σeq .varSort e w'))
      → clo e (var w) ρ ≡ ρ w
  ...
  module _ {V : Type ℓv} {vs : V → S} where
    ...
    opCong : (o : σ .ops)
      {g h : (a : arities σ o) → FreeModel V vs (σ .sortOf o a)}
      → ((a : arities σ o) → g a ≡ h a)
      → Path (FreeModel V vs (σ .resultSort o)) (node o g) (node o h)
    opCong o p i = node o (λ a → p a i)

    cloTmRec : (e : σeq .eqns)
      (ρ : (w : vars σeq e) → FreeModel V vs (σeq .varSort e w))
      {s : S} (M : Tm σ (vars σeq e) (σeq .varSort e) s)
  ...
  module _ {X : S → Type ℓX} (isSetX : (s : S) → isSet (X s))
    (α : Ops {σ = σ} X)
    (sat : (e : σeq .eqns)
           (ρ : (w : vars σeq e) → X (σeq .varSort e w))
    ...
    recHomo : {V : Type ℓv} {vs : V → S} (ρ : (v : V) → X (vs v))
      (o : σ .ops)
      (x : (a : arities σ o) → FreeModel V vs (σ .sortOf o a))
    ...
    module _ {V : Type ℓv} {vs : V → S} (ρ : (v : V) → X (vs v))
      ...
      uniqNode : (o : σ .ops)
        (ts : (a : arities σ o) → FreeModel V vs (σ .sortOf o a))
        (ih : (a : arities σ o)
...
-- The dependent eliminator.  `rec` is non-dependent, and transporting a
-- dependent statement along `recUniq` is opaque; this is the general form,
-- with `elimProp` as the corollary every client so far wants.
module _ {S : Type ℓS} {σ : SortedSig S ℓ} (σeq : SortedEqns σ ℓ'')
  {V : Type ℓv} {vs : V → S}
  {ℓP : Level} {P : {s : S} → FreeModel σeq V vs s → Type ℓP}
  where

  module _
    (isSetP : {s : S} (m : FreeModel σeq V vs s) → isSet (P m))
    (pvar : (v : V) → P (var v))
    (pnode : (o : σ .ops)
    ...
    (pcloVar : (e : σeq .eqns) (w : vars σeq e)
        (ρ : (w' : vars σeq e) → FreeModel σeq V vs (σeq .varSort e w'))
    ...
    elim : {s : S} (m : FreeModel σeq V vs s) → P m
    elim (var v) = pvar v
    ...
    elim (trunc m m' p q i j) =
      isOfHLevel→isOfHLevelDep 2 (λ x → isSetP x)
        (elim m) (elim m') (cong elim p) (cong elim q) (trunc m m' p q) i j
  ...
  module _
    ...
    elimProp = elim (λ m → isProp→isSet (isPropP m)) pvar pnode pclo
      (λ e w ρ pρ → isProp→PathP (λ i → isPropP (cloVar e w ρ i)) _ _)
      (λ e o ts ρ pρ → isProp→PathP (λ i → isPropP (cloNode e o ts ρ i)) _ _)
      (λ e ρ pρ → isProp→PathP (λ i → isPropP (eqn e ρ i)) _ _)
