...
{- `Combinator/Core` at `ND`: every answer.  Completeness is a claim about the
   grammar, not the type -- nothing types "these are all of them". -}
...
-- a node's enumeration is the cartesian product of its slots'
...
-- the list isomorphism, used only to reach `allΠFin` and come back
...
-- derivations of `A` at `f m` are derivations of `A ∘ f` at `m`
NDAnswer .AnswerFunctor.Ans-re f m x = ndFromList m (ndToList (f m) x)

-- Routing at `ND` *counts*, so it detects a route whose `disjoint` is a lie;
-- see `Instances/Class/Resolve`.
...
-- `observe` at `ND`: the action is applied under the list, once per derivation.
