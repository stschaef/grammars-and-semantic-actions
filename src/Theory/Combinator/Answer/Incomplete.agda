...
{- `Combinator/Core` at `Maybe`: at most one answer, no refutation.  `Ans-⊕&`
   is `orElse`, committing left -- the PEG reading, so ambiguity silently
   loses derivations.  `Ans-node` ignores `Precise`: with nothing to refute,
   a node needs only the traversal. -}
...
-- `Maybe` is covariant: `nothing` answers every grammar, so routing is
-- cheap -- cells the route did not name are never consulted and owe no
-- refutation.
