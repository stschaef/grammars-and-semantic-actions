...
-- `Combinator/Core` at `Dec`: a checker built from these combinators is
-- sound and complete by typing rather than by a theorem.
...
-- the whole of `Ans-node`'s search: a node has no other quantifier
...
DecAnswer .AnswerFunctor.Ans-node o prec {As = As} {ms = ms} ws =
  ...
  -- `Precise`: every decomposition is `ms`, so one slot refutation refutes
  -- the node
...
-- Exactly `routeIn`.  `DecAnswer` has no `CovariantAnswer` (`Ans-empty`
-- would be a uniform decision procedure), so no `FromCov` derivation.
