...
{- STLC with annotated application; operations indexed by external data
   (`appOp B` = "apply at argument type `B`") make it syntax-directed:

     Γ ⊢ var x        ⇐ A   iff  Γ(x) = A
     Γ ⊢ app[B] f a   ⇐ A   iff  Γ ⊢ f ⇐ B ⇒ A  and  Γ ⊢ a ⇐ B
     Γ ⊢ lam[B] x t   ⇐ A   iff  A = B ⇒ C  and  Γ,x:B ⊢ t ⇐ C

   Premise types are determined by conclusion plus annotation, so no
   synthesis mode; dropping `B` from `appOp` is where `Route` is needed. -}
...
-- types annotate operations rather than occupying a sort, so type
-- equality stays a plain decidable equality
infixr 25 _⇒_
data Ty : Type ℓ-zero where
  ι : Ty
  _⇒_ : Ty → Ty → Ty

-- "A is an arrow with domain B" as `IsArr A × (dom A ≡ B)`: a decidable
-- proposition, usable as a slot of a node
...
-- term model as a plain datatype, so everything reduces
