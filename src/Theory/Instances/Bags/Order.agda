...
-- ⊤ at ε, a pair at ⊙, p at a generator -- on the nose (`Cl.rec` reduces)
...
-- `nothing` is +∞: a lower bound only the empty bag meets, pinning the
-- accumulator to `ε` at the top call.
...
-- `bagAll` is a fold, so the intro/elim rules hold on the nose: nothing
-- is transported.
private variable ℓA ℓB : Level

module _ (p : El → hProp ℓ-zero) where
  -- witness carried, never matched: a prop-valued fold moves it along the
  -- splitting on the nose
  ...
  -- backwards: at a generator the fold *is* `p`, so the witness reads back
  -- out as a constant
  bagAll-atGen : (y : El) → ⌈ ⌈gen y ⌉ ⌉ & bagAll p ⊢ K ⟨ p y ⟩ & ⌈ ⌈gen y ⌉ ⌉
  bagAll-atGen y m (Eq.refl , q) = q , Eq.refl

-- Folds agreeing on generators are equal: `bagAll` commutes with ⊓ by
-- uniqueness of homomorphisms, no induction over bags.
...
-- Monotone, structurally by the prop-eliminator: via `bagAll-⊓` the
-- `⇔toPath` transport does not reduce even at a single generator.
private
