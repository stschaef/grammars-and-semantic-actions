{- Joining two sorted halves around a pivot.  A recursion on the left half
   whose motive is the multiplicative residual `⊸B`: the right half is not
   quantified over, it is what the residual abstracts.

   The recursion is löb on bag size rather than structural on `Sorted`.  It
   *is* structural -- the left half's tail is one generator shorter -- but a
   function-typed motive does not reduce through `μ`'s recursor, while a
   function-typed löb family does. -}
...
module _ (x : El) where
  ...
  private
    -- every element of the pivot bundle is above `y`: the pivot by `w`, the
    -- half above the pivot by transitivity
    ...
    -- the ε is absorbed into the pivot's own index, so the sorted half
    -- above the pivot is never transported
    nilJ =
      ⇒-intro (⊸B-intro
        ( consSorted x
        ∘⊢ ⊎Bmap ⊎B-unitL⌈⌉ id⊢
    ...
    consJ y = ⇒-intro (⊸B-intro
      ...
      ∘⊢ ⊎Bmap pre id⊢ ))
      where
      -- put the guard on the tail, then split the bound along the tensor
