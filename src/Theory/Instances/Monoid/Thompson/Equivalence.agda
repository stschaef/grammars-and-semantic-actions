...
{- Thompson's theorem: the parses of `regex→NFA r` are the parses of `r`.
   ...
   Each clause is its construction's isomorphism composed with the
   congruence of the connective it introduces.  `⟦_⟧nfa` is `Regex.Base`'s
   `⟦_⟧` with the lifts a trace carries: a trace's literals sit at the
   automaton's level, the regex's at `ℓM`. -}
