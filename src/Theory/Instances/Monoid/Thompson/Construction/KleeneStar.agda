...
{- Thompson's construction at `*`.  The folds `from*NFA` and `to*NFA` are
   bridged by `nested-induction`. -}
...
module _ (N : NFA ℓN) where
  ...
  *NFA .ε-dst (N-internal t) = Sum.inr (N .ε-dst t)
  ...
  -- what a state owes: the rest of the list, and at `N`'s states also the rest of that parse
  ⟦_⟧* : Unit Sum.⊎ ⟨ N .Q ⟩ → TheoryTy ℓ* tt
  ⟦ Sum.inl _ ⟧* = A.Parse *
  ⟦ Sum.inr q ⟧* = A.Trace q ⊗ (A.Parse *)

  *NFAAlg : TraceAlg ⟦_⟧*
  -- `NIL`/`CONS` spelled through `⟦⊗e⟧` so the laws below need not unbundle `two`.
  ...
  *NFA-ret = rec-section TraceTy *NFAAlg to*NFA to*NFA-homo
  ...
  -- Bridge between the folds, curried so the induction is on `N`'s trace alone.
  private
