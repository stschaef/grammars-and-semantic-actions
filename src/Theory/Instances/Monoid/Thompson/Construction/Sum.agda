...
{- Thompson.s construction at `⊕`: a fresh initial state with a silent
   transition into each automaton; both round trips are `rec-section`. -}
...
module _ (N : NFA ℓN) (N' : NFA ℓN') where
  ...
  private
    -- naming the composite a `step` branch unfolds to gives `cong` a domain to solve for
  ...
  -- `toNFA` is an algebra map: every branch is `refl` except the labelled
  -- transition, which is `map-step` on each side.
  ...
  -- `fromNFA` is one too; `rec-section` at `N` then makes the round trip
  -- the identity on `N`.s traces.
  private
