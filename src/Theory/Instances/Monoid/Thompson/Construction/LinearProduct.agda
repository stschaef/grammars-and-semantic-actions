...
{- Thompson's construction at `⊗`: the automata side by side, one silent
   transition out of each accepting state of `N` into `N'`'s start.  The
   crossover state is not input-determined, so the map back folds `N`'s trace
   into a `⊗NFA` trace still OWING an `N'` trace, and the crossing spends it. -}
...
module _ (N : NFA ℓN) (N' : NFA ℓN') where
  ...
  -- what is still owed from each state
  ...
  -- the residual carries the nondeterminism of the crossover
  ⟦_⟧N : ⟨ N .Q ⟩ → TheoryTy ℓ⊗ tt
  ...
  -- One `rec-section` covers both states: `equalizer-ind` forces a carrier
  -- at every state; a homomorphism law does not.
  gmap : ∀ q → ⟦ q ⟧⊗ ⊢ Trace q
  ...
  private
    ...
    -- the bodies `NAlg` curries, written out so `⟜-β` applies
  ...
  -- The section is proved in CURRIED form: the `N'` parse the crossing
  -- spends is abstracted with `⟜`, so induction runs on `N`'s trace alone;
  -- `⟜-η` is `refl`, so `cong ⟜-intro` moves branches back uncurried.
  private
    C⟜ : ⟨ N .Q ⟩ → TheoryTy ℓ⊗ tt
    C⟜ q = (A.Trace q ⊗ A'.Parse) ⟜ A'.Parse

    -- `fromNFA` sits outside the residual, so uncurrying a branch leaves it
    -- outside too (`⟜-post-intro⁻`).
