...
{- Thompson's construction at `satr`: `literalNFA` with `Unit` replaced by
   `Sat P` -- one transition per satisfying character, which is why the
   alphabet must be finite (an NFA's transitions are a `FinSet`). -}
