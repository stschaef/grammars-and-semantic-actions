...
{- The theory of monoids, freely extended by further equations over the
   same signature, and everything the monoid equations alone give.

   `MonPlusEqns E …` takes `MonEqns`' three equations *literally* -- `mon`
   is the inclusion and its `varCount`/`lhs`/`rhs` are `MonEqns`' fields on
   the nose -- and adjoins `E`.  So an extension neither restates nor has
   to re-prove the monoid laws: `⊗ₑ`, its unitors and associator, the
   residual `⊸ₑ`, and the carrier-level `⊙-assoc`/`⊙-unitL`/`⊙-unitR` are
   proved once here and inherited.

   `Instances/Bags` is the extension by a single permutation equation; it
   adds `⊙-comm` and nothing else.

   `Instances/Monoid` is *not* an instance, for two reasons.  This module
   would have to import it for `MonEqn`, so `MonEqns = MonPlusEqns ⊥` is
   circular; and more substantially, the monoid instance is presented by
   lists, where `op _⊙_ ms` reduces to `_++_`, and its `⊗-assoc` and
   unitors are written against that reduction rather than transported
   along the equations.  Everything downstream of `Strings` depends on it.
   This module is presented by `closingPresentation`, where nothing
   computes and transport is the only option. -}
...
-- The carrier as a monoid.  These are the equations read off the
-- presentation, with the `two`/`three` tuples η-expanded.
...
-- Associativity of `⊗ₑ`, by transport along the `assoc` equation.  The
-- glue on either side is the passage between a two-slot tuple and the
-- term the equation is stated with.
...
-- The residual, right adjoint to `_⊗ₑ_` in its second argument.
