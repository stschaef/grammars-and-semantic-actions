...
{- `⊕[ b ] Trace b q` is a proposition, which makes `parse`/`print` inverse.
   Induction on one trace against a one-step unrolling of the other. -}
...
module _ {Q : Type ℓQ} (Aut : DeterministicAutomaton Q) where
  ...
  module _ (b b' : Bool) where
    private
      ...
      -- same letter, so the recursive call at the common successor decides
