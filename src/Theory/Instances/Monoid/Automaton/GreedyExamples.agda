{- The OLD scan, kept honest.
   ...
   `Automaton/GreedyMax` supersedes `Automaton/Greedy` in every respect --
   its type says the match is maximal, and it is faster -- and
   `GreedyMaxExamples` carries the scale numbers and the map into the
   word-indexed specification.  What that file cannot do is pin THIS
   module's `scan`, which nothing else exercises computationally.

   So: the four behaviours, over `Automaton/Greedy.scan`, and nothing
   else.  If `Automaton/Greedy` is ever retired, this goes with it.
