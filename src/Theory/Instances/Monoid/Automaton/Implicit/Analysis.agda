...
-- First/follow analysis, `RE` to `DetReg`.  `Disj` is finite only when at
-- most one side is a class: `[a-z]|[0-9]` is refused though deterministic.
...
open import Cubical.Relation.Nullary.DiscreteEq using (DiscreteEq→Discrete)
...
-- not `public`: re-exporting the constructors makes them ambiguous in clients

private variable
  n n' : Nullability
  nn nn' : Bool

-- `with` cannot see through a definition, so eliminations are `where` on
-- the sum.

discAlphabet : Discrete Alphabet
discAlphabet = DiscreteEq→Discrete _≟_
...
-- the `(v : Bool) → x ≡ v` idiom inspects a stuck `Bool` with its equation
...
-- `Supp` makes a predicate's complement decidable outside a small
-- exception set.
...
private
  ...
  -- class/class cannot be settled: neither predicate is enumerable
...
-- Leaves never step after accepting, so follow-last supports are empty.
...
-- `⊕DR`'s follow-last index depends on nullability: non-nullable branches
-- cannot be re-entered, so their first sets stay out of it.
...
-- The loop re-enters at the first set, so it joins the follow-last index.
...
-- The `εr` clauses matter: `Regex.Parse` builds every concatenation as
-- `εr ⊗r …`, and `⊗DR` refuses a nullable left factor.
...
-- The `⊕` clause splits on `b` only because `νOf b +ν νOf b'` is stuck
-- for a variable `b`.
...
-- A rejected regex is a type error at the use site, not a `nothing` to
-- case on.
