...
{- Maximal munch with maximality in the type: `TraceTo q q'` fixes the end
   state (`Greedy`'s existential is unconstrained), and extending is O(1). -}
...
open import Theory.Instances.Monoid.Automaton.Greedy Alphabet isSetAlphabet
  using (¬Nullable-map ; &-¬NullableR ; ⊕ᴰ-¬Nullable ; char⁺-¬Nullable
       ; ¬Nullable→¬ε ; ¬Nullable→char⁺ ; noExt-ε ; module States)
...
module _ {Q : Type ℓAlph} (M : DeterministicAutomaton Q) where
  ...
  -- `Deterministic`'s `Tag`, acceptance equation replaced by an end-state
  -- equation: stopping is allowed only there.
  ...
  -- a run from `q` ending at `q'`; the end state is the parameter, the
  -- start the index
  ...
  -- `Trace` inverted at each step via `flatEq` + cons-injectivity, as in
  -- `Automaton/Unambiguous`.
  ...
  private
    ...
    -- the recursive call arrives as `kont`; a `where` binding would hide
  ...
  -- A dead `δ q c` is refuted without projecting the tail's cell: forcing
  -- it would make one token cost the whole remaining input.
  ...
  scan-cons D = &ᴰ-intro λ q → ⊕ᴰ-elim (λ c → stepAt q c) ∘⊢ ⊗⊕ᴰ-distL
    ...
    stepAt q c = alive (isDead (δ q c)) Eq.refl
      ...
      -- dead: refute from the certificate; the tail is discarded, not projected
  ...
  module _ (isSetQ : isSet Q) where
    ...
    -- `Automaton/Unambiguous`'s induction with the end-state equation;
    -- with `endState` it makes `BridgeTy b q` a proposition.
    ...
    Trace≅TraceTo b q m .Iso.sec t = λ i → bridge-section b q i m t
    Trace≅TraceTo b q m .Iso.ret s = λ i → bridge-retract b q i m s

  -- Maximality cashed in: `cancel` moves a run past the match, to where the refutation lives.
