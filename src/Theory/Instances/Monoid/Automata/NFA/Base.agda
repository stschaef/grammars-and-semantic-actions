...
record NFA ℓN : Type (ℓ-suc (ℓ-max ℓN ℓAlph)) where
  ...
  module Accepting where
    ...
    -- `k (Lift εTy)` rather than `⊗e ε·`: `Lift` has η, so `map` at a `k`
    -- summand is definitionally the identity and every stop branch of an
    -- `equalizer-ind` below closes by `refl`.
    ...
    module _ {ℓB} {A : States → TheoryTy ℓB tt} where
      ...
      step-β : ∀ {q} t (p : src t Eq.≡ q) → step-out t p ∘⊢ step-in t p ≡ id⊢
      step-β t p = refl
