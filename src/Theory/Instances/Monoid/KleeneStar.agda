...
-- Right-nested lists of parses; the branch stays a named function so
-- repeated occurrences compare directly.
...
-- One-step unfolding, inverse to `⊕-elim CONS NIL`.  `fold*r` cannot supply
-- it: a parser.s `mapP` needs the grammar back out, not a fold over it.
UNCONS-branch : {ℓA ℓB : Level} {A : TheoryTy ℓA tt}
  {B : Unit → TheoryTy ℓB tt}
  → ⟦ starBranch A true ⟧TheoryTy B ⊢ A ⊗ B tt
...
-- `starBranch char b` and `kleeneBranch b` agree at each `b` but the two
-- `Bool → Functor` functions never unify; the fold renames the summand, and
-- `read` stays the only recursion on a word.
fromString* : String* ⊢ char *
fromString* = rec (λ _ → KleeneCode) alg tt
  where
  alg : ∀ _ → ⟦ KleeneCode ⟧TheoryTy (λ _ → char *) ⊢ char *
  alg _ = roll ∘⊢ ⊕ᴰ-elim λ where
    false → σ⊕ false
    true  → σ⊕ true

readChars : ⊤Ty ⊢ char *
readChars = fromString* ∘⊢ read
...
-- `roll*`/`unroll*` (and the unlifted `roll↑`/`unroll↑`) are inverse: the
-- wrapper costs `two-η` at cons (`Fin 2` has no η) and `funExt λ ()` at nil.
