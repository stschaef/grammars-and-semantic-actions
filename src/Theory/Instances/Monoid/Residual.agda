...
{- Residuals of `_⊙_` and convolution plumbing; nothing downstream of this
...
-- Build every cast in `Eq` and match it: `Eq.pathToEq` is `JPath`, which
-- does not reduce even on `reflPath`, so path-built casts block evaluation.
castEq : {A : ↓M tt → Type ℓ} {x y : ↓M tt} → x Eq.≡ y → A x → A y
castEq Eq.refl a = a

-- the index is a set, so which path the `PathP` is taken along never matters
...
-- `Resid` at slot `zero`, written out so the unused slot level needs no `Lift`.
...
-- Transpose of `(A ⊗ -) ⊣ (A ⊸ -)`; `A` is the *left* factor.
module _ {A : TheoryTy ℓ tt} {B : TheoryTy ℓ' tt} {C : TheoryTy ℓ'' tt} where
  ⊸-lam : A ⊗ B ⊢ C → B ⊢ A ⊸ C
...
-- Currying reverses: `A`, the earlier word, ends up innermost.
⊸-curry : {A : TheoryTy ℓ tt} {B : TheoryTy ℓ' tt} {C : TheoryTy ℓ'' tt}
  → (A ⊗ B) ⊸ C ⊢ B ⊸ (A ⊸ C)
...
-- Unlike `Strings.⊗-unit-l` (built via non-reducing `pathToEq`), this one
-- matches `Eq` equations, so parsers built on it compute.
...
-- The four triangles live here, not by `⊗-pent`, since they mention `εTy`'s unitors.
...
open import Theory.Type.Distributivity MonEqns Alphabet (λ _ → tt)
  listPresentation using (&⊕ᴰ-distR ; &⊕ᴰ-distL)
