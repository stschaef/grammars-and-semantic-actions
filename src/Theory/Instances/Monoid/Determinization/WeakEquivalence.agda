...
{- The subset construction, and the weak equivalence it produces.

   Determinisation is *weak*: the DFA has one run where the NFA had many, so
   the two agree on which words are accepted and not on how many parses they
   admit.  That is exactly what `_≈_` records.

   The states are required to sit at `ℓ-max ℓN ℓAlph`.  This is not a level
   hack: the powerset the construction ranges over has to hold the transition
   relation, whose decidable proposition compares *labels*, so it cannot be
   smaller than the alphabet.  Any `NFA ℓN` can be brought into this shape by
   lifting its carriers. -}
...
open SortedSig
open SortedEqns
module Theory.Instances.Monoid.Determinization.WeakEquivalence
  {ℓAlph}
...
open import Theory.Instances.Monoid.Automata.DFA.Base Alphabet isSetAlphabet
open import Theory.Type.Equivalence.Base MonEqns Alphabet (λ _ → tt)
  listPresentation using (_≈_ ; mkWeakEq)
...
module Determinization
  ...
  is-ε-closed : ⟨ FinSetDecℙ N.Q ⟩ → Type ℓQ
  is-ε-closed X =
  ...
  ℙNAcc-DecProp' : (X : εClosedℙQ) → DecProp' ℓQ
  ℙNAcc-DecProp' X =
    DecProp'∃ N.Q
      (λ q → DecProp'×
    ...
    ⊕ᴰ-elim (λ {
        NTrace.stop → ⊕ᴰ-elim (λ {
          (lift acc) → &ᴰ-intro λ X →
            &ᴰ-intro (λ q∈X →
              ℙN.STOP' (embedAcc q X q∈X acc) ∘⊢ lowerTy ∘⊢ lowerTy
            ) })
      ; NTrace.step → ⊕ᴰ-elim (λ { (t , Eq.refl) →
        &ᴰ-intro (λ X → &ᴰ-intro λ src∈X →
          ℙN.STEP {b = true} (N.label t) X ∘⊢
          id⊢ ,⊗ (π (lit-closure-transition t X src∈X) ∘⊢
                  π (ε-closure (lit-closure (N.label t) (X .fst)))) ∘⊢
          NTrace.step-out t
          )
          })
      ; NTrace.stepε →
        ⊕ᴰ-elim λ { (t , Eq.refl) →
          &ᴰ-intro (λ X → &ᴰ-intro (λ src∈X →
            π (ε-closure-transition t X src∈X) ∘⊢
            π X)) ∘⊢
          lowerTy } })
  ...
  DFA→NFA-alg :
    ℙN.TraceAlg true
      (λ X → ⊕[ q ∈ ⟨ N.Q ⟩ ] ⊕[ q∈X ∈ q ∈ε (X .lower) ] NTrace.Trace true q)
  DFA→NFA-alg (lift X) =
    ⊕ᴰ-elim (λ {
      (stop accX) →
        let
    ...
    step-help c q X q∈εlitX q' q'∈litX n walk t qt qt∈x
      ...
      NTrace.STEP true t ∘⊢
      id⊢ ,⊗ fold-walk q (lit-closure c X) q' q∈εlitX (n , walk)

  DFA→NFA : ∀ X →
    ℙN.Trace true X ⊢
      ⊕[ q ∈ ⟨ N.Q ⟩ ]
      ⊕[ q∈X ∈ q ∈ε X ] NTrace.Trace true q
  DFA→NFA X = rec (ℙN.TraceTy true) DFA→NFA-alg (lift X)
