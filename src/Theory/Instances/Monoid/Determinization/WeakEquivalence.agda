...
{- Subset construction, weak: the DFA agrees with the NFA on acceptance,
   not parse count (`_≈_`).  States sit at `ℓ-max ℓN ℓAlph` because the
   powerset must hold the label-comparing transition relation. -}
...
module Determinization
  ...
  opaque
    -- decidable finite set of states reachable from q-start
