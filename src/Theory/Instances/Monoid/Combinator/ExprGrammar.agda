...
{- The expression grammar, shared by the two parsers built for it.

     E → E + T | T          T → ( E ) | x

   Left-recursive and mutually recursive.  Nothing here is about parsing:
   this is the alphabet, the grammar as a `μ`, its four productions as
   `⊢`-terms, and the readout into an ordinary datatype.  `Ascent/Expr`
   parses it with the ascent combinators; `LeftCorner/Expr` parses a
   left-corner transform of it with `Decidable/Productions` and rebuilds the
   value.  Both import this. -}
...
-- THE GRAMMAR the parser is *for*: left-recursive, mutually recursive.
data NT : Type where
  nE nT : NT

-- `G` is exactly the shape `Derivative/Parser` takes a grammar in, so it is public:
-- the derivative construction applies to this grammar unchanged.
