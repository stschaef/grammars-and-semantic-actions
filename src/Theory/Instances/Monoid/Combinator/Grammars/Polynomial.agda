...
{- Parenthesised multivariate polynomials, one recursive-descent parser.

     E ::= ( E ) K | v K | n Q | - E
     Q ::= / m K | + E | * E | ^ n K | ε
     K ::=         + E | * E | ^ n K | ε

   `E` routes on one token; `K`/`Q` route on the head only (`caret`/`slash`
   route again over ℕ); the nullable ε-production is `_<|>_`d on instead. -}
