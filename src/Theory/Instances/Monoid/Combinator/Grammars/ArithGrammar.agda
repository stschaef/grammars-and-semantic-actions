...
{- Arithmetic expressions.
   ...
   Every production begins with a terminal, so one token names the production. -}
...
isSetTag N = DiscreteEq→isSet (decTag N)
...
-- `rExp` is the LL(1) table for `Exp`, `gExp` the coarsening of the one-token
-- cover along it; both are answer-free -- a table is a table at every answer.
