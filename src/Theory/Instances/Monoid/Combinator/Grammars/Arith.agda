...
{- Arithmetic as one recursive-descent parser, answer-generic.  `Exp` is
   ROUTED -- one token names the branch; `Exp'` is nullable, so its ε-branch
   cannot be predicted and is `_<|>_`d.  Instantiations: `Grammars/ArithTests`. -}
