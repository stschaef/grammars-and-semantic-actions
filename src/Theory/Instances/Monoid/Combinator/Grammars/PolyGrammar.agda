...
{- Parenthesised multivariate polynomials; variables, numerals, exponents are single tokens.

     E ::= ( E ) K | v K | n Q | - E
     Q ::= / m K | + E | * E | ^ n K | ε
     K ::=         + E | * E | ^ n K | ε

   Every production begins with a terminal (`call` answers only at proper suffixes);
   left-factored so one token decides every production (FIRST(Q) disjoint from FOLLOW(Q)). -}
...
open import Cubical.Relation.Nullary.DiscreteEq using (discreteEqCong)
...
private
  -- structural recursion, not `discreteℕ`: `Eq.pathToEq` yields a non-`Eq.refl`
  -- proof, so `dec-lit⊗-at` gets stuck and a matched numeral never computes
  _≟ℕ_ : (a b : ℕ) → (a Eq.≡ b) Sum.⊎ ((a Eq.≡ b) → Empty.⊥)
  zero ≟ℕ zero = Sum.inl Eq.refl
  zero ≟ℕ suc b = Sum.inr λ ()
  suc a ≟ℕ zero = Sum.inr λ ()
  suc a ≟ℕ suc b = discreteEqCong suc (λ where Eq.refl → Eq.refl) (a ≟ℕ b)

-- `Eq.≡` rather than a path: `Sum.inl Eq.refl` makes a matched letter compute
...
var v ≟ var w = discreteEqCong var (λ where Eq.refl → Eq.refl) (v ≟V w)
nat n ≟ nat n' = discreteEqCong nat (λ where Eq.refl → Eq.refl) (n ≟ℕ n')
lp ≟ rp = Sum.inr λ ()
lp ≟ plus = Sum.inr λ ()
lp ≟ times = Sum.inr λ ()
lp ≟ slash = Sum.inr λ ()
...
-- A tag is a *sum*, not a `data`, so `isSet` is compositional.
...
-- `E` routes on one token; `pow n`/`den m` carry a numeral from the *second* token,
-- so no `M₁ → Maybe KTag` exists.  Repair: route on the head (`caret`, `slash`),
-- then route again over `ℕ` (`PowSet` is that inner sum).
...
-- `KH` is `KTag` with the numeral factored out and ε dropped -- what one token
-- can name; `QH` adds the denominator's head.
...
-- `ty (CE t)` is `BodyExpr t` on the nose.
...
-- Regluing: `E` is an exact relabelling; `K`/`Q` are their non-nullable heads plus ε,
-- with the `pow`/`den` inner `⊕[ n ∈ ℕ ]` redistributed across the leading letter.
