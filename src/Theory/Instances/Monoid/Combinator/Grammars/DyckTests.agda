...
{- `Grammars/Dyck` written once, run at all three answers.

   Nothing below re-states the grammar: `GDec.dyck`, `GInc.dyck` and
   `GND.dyck` are the same `step` at three `AnswerFunctor`s.  The decider
   refutes, the `Maybe` parser declines, and the enumerator returns the empty
   list -- and on a well-bracketed word all three agree, the enumeration
   having exactly one element because `S` is unambiguous. -}
open import Cubical.Foundations.Prelude

module Theory.Instances.Monoid.Combinator.Grammars.DyckTests where
