...
{- A grammar family needing exactly `k` tokens of lookahead:

     S ::= aᵏ⁺¹ S b | aᵏ⁺¹ c

   Both productions lead with the same `k+1` letters, so only width `k+2`
   separates them. -}
...
module Gram (kk : Width) where
  ...
  isSetSys St = lift isSetTg , λ where
    ...
    where
    isSetTg : isSet (Tg St)
    isSetTg = DiscreteEq→isSet (decTg St)
