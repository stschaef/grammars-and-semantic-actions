...
{- Predictive parser generator.  `Prod o` is indexed by the class it predicts,
   so a table cannot conflict: the LL(1) condition is what a table *is*. -}
...
-- a body may only be given for the class its leading terminal names
...
-- the class names the production, so a rose tree loses nothing
...
module Gen {X : Type ℓAlph} (T : Table X) where
  ...
  -- Each production claims the class its leading terminal names; a class with no production claims it vacuously.
  ...
  -- `&ᴰ` bundles the family so Löb is taken once; `call` reads a component at a strict suffix.
