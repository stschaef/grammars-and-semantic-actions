...
decMTok (just x) (just y) = go (x ≟T y)
  where
  go : (x Eq.≡ y) Sum.⊎ ((x Eq.≡ y) → Empty.⊥)
...
decCls (after s) (after t) = go (decMTok s t)
  where
  go : (s Eq.≡ t) Sum.⊎ ((s Eq.≡ t) → Empty.⊥)
     → (after s Eq.≡ after t) Sum.⊎ ((after s Eq.≡ after t) → Empty.⊥)
  go (Sum.inl Eq.refl) = Sum.inl Eq.refl
  go (Sum.inr ne) = Sum.inr λ where Eq.refl → ne Eq.refl
decCls (headed c) (headed d) = go (c ≟T d)
  where
...
-- a bracket opened, then closed by the body itself
...
-- ...and for a word that swallows its own closing bracket
...
-- and the index is discrete, which is the other thing `PushOf` asks for
...
-- a letter that is not a bracket
...
-- ...and a bracket closed by a following `)`
