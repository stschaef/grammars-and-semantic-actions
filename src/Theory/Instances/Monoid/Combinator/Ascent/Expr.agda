...
{- An LR parser for the expression grammar, written with the ascent
   combinators and nothing else.

     E → E + T | T          T → ( E ) | x

   The parser shifts, gotos and reduces: `shift` puts a token on the `⟜`
   tower, `goto` composes two states' contributions, `reduce` applies a
   production term, `runA` feeds the empty stack.  There is no table, no
   stack of states, and no `Decidable/Productions`.

   The automaton's own grammar is the one an LR machine walks -- left corners
   factored out so every state consumes a token before it recurses -- and the
   *value* is a parse of the original left-recursive `E`, assembled by the
   reductions.  `Mot sR = E ⊸ E` is the stack of pending reductions and
   `⊸-app` is the reduction firing. -}
...
module Theory.Instances.Monoid.Combinator.Ascent.Expr where
...
-- THE AUTOMATON'S OWN GRAMMAR: left corners factored out, so every state
-- consumes a token before it recurses.  This is what an LR machine walks.
data SK : Type where
  sE sR sT : SK
...
-- THE PARSER.  `shift` eats a token onto the stack, `goto` composes, `reduce`
-- fires a production -- and `chooseA` commits on one token of lookahead, so
-- no branch is ever attempted speculatively.
private
  ℓb : Level
  ℓb = ℓ-suc ℓ-zero
  ...
  -- one branch per lookahead class
  ...
  -- the class sum, rolled back into the μ
...
-- WHAT THE REDUCTIONS BUILD.  `Mot sR = E ⊸ E` is the stack of pending
-- reductions: an `E` is owed on the left, and `⊸-app` is the reduction
-- firing.  That is why `x + x + x` comes out left-associated.
...
-- THE PARSER: run the ascent, then read the original grammar's tree.
parse : Test E
...
-- THE LR CONDITION, as a checked term rather than a side note: two branches
-- at different lookahead classes cannot both match the same word.  This is
-- what `_<|>_` never had to prove, and it is what makes the commitment above
