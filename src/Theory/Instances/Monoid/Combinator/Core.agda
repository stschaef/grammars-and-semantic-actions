...
{- The combinators, once, over `AnswerFunctor`.  `mapP`/`fail` need variance
   (`CovariantAnswer`); `CommittingAnswer` is the LL layer's extra requirement. -}
...
open import Theory.Instances.Monoid.Residual Alphabet isSetAlphabet
  using (⊗ε-unit-l⁻ ; ⊗ε-unit-r ; ⊗ε-unit-r⁻ ; ⊗⊕ᴰ-distL)
...
-- here, not `Decidable/Routed`: every answer routes; only the *commitment* differs
...
decM₁ (tk c) (tk d) = discreteEqCong tk (λ where Eq.refl → Eq.refl) (c ≟ d)
...
-- re-bundled at the concrete type: the wildcat's grammar `≅` is a private module there
...
record AnswerFunctor : Typeω where
  field
    ...
    -- `Dec` needs the backward map to move a refutation; `Maybe`/`ND` ignore it.
    -- No combinator ever has two independent maps: always one isomorphism.
...
-- exists because `Dec`'s `mapP` is genuinely divariant: a decision transports along a retract
...
-- At `Dec` the branch may come back `no`; the cover's `disjoint` turns that into a refutation
-- of the whole sum.  A covariant answer instead drops untaken cells (`FromCov.committing`).
...
-- kept separate so sketching a new answer does not owe the functor laws up front
