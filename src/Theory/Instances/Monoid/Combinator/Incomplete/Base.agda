...
{- `Core`'s combinators at `Maybe` (the Danielsson/Allais adaptation).
   `Ans-⊕&` is `orElse`, committing to the left summand -- the whole
   difference from `NonDet`, whose `Ans-⊕&` is `appendND`. -}
...
-- the strength; `NonDet`'s counterpart has to traverse
...
-- Derived by discarding: `Ans-route` answers `nothing` at the cell not
-- taken, so routing teaches `Maybe` nothing `_<|>_` would not.
