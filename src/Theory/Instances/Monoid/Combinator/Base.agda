...
module Theory.Instances.Monoid.Combinator.Base
  {ℓAlph}
  (Alphabet : Type ℓAlph)
...
▷dec-map : {b : Bool} {K : TheorySet ℓK tt} {L : TheorySet ℓL tt}
  → ty K ⊢ ty L → ty L ⊢ ty K
  → ty (▷? b (DecSet K)) ⊢ ty (▷? b (DecSet L))
▷dec-map f g = ▷map (dec-map f (¬Ty-map g))

▷dec-⊕& : {b : Bool} {K : TheorySet ℓK tt} {L : TheorySet ℓL tt}
  → ty (▷? b (DecSet K)) & ty (▷? b (DecSet L))
  ⊢ ty (▷? b (DecSet (K ⊕Set L)))
▷dec-⊕& = ▷map dec-⊕& ∘⊢ ▷lax
...
-- decisions on A ⊗ K
-- Parsers are indexed by two booleans that indicate which resources
-- the domain an codomain refer to
-- ▷? true A = ▷ A
-- ▷? false A = ▷ A & A ≅ □ A
Parser : Bool → Bool → TheorySet ℓA tt → TheoryTy _ tt
Parser a c A =
  &[ K ∈ TheorySet ℓG tt ]
    (ty (▷? a (DecSet K)) ⇒ ty (▷? c (DecSet (A ⊗Set K))))

ParserSet : (a c : Bool) → TheorySet ℓA tt → TheorySet _ tt
ParserSet a c A =
  Parser a c A , isSet&ᴰ λ K → isSet⇒ (▷? c (DecSet (A ⊗Set K)) .snd)

mkP : {a c : Bool} {A : TheorySet ℓA tt} {D : TheoryTy ℓD tt}
  → (∀ K → D & ty (▷? a (DecSet K)) ⊢ ty (▷? c (DecSet (A ⊗Set K))))
  → D ⊢ Parser a c A
mkP f = &ᴰ-intro λ K → ⇒-intro (f K)

pAt : {a c : Bool} {A : TheorySet ℓA tt} {D : TheoryTy ℓD tt}
  → D ⊢ Parser a c A → (K : TheorySet ℓG tt)
  → D & ty (▷? a (DecSet K)) ⊢ ty (▷? c (DecSet (A ⊗Set K)))
pAt p K = ⇒-app ∘⊢ ((π K ∘⊢ p) ,&p id⊢)

pmore : {c : Bool} {A : TheorySet ℓA tt} → Parser true c A ⊢ Parser false c A
pmore = mkP λ K → pAt id⊢ K ∘⊢ (id& ▷wk)

pless : {a : Bool} {A : TheorySet ℓA tt} → Parser a false A ⊢ Parser a true A
pless = mkP λ K → ▷wk ∘⊢ pAt id⊢ K

mapP : {a c : Bool} {A : TheorySet ℓA tt} {B : TheorySet ℓB tt}
...
pApp : {A : TheorySet ℓA tt} (K : TheorySet ℓG tt)
  → ty (▷ (ParserSet false false A)) & ty (▷ (DecSet K))
  ⊢ ty (▷ (DecSet (A ⊗Set K)))
...
module _ {D : TheoryTy ℓD tt} where
  ...
  seq : {a b c : Bool} {A : TheorySet ℓA tt} (B : TheorySet ℓG tt)
  ...
  _<|>_ : {a c : Bool} {A : TheorySet ℓA tt} {B : TheorySet ℓB tt}
    → D ⊢ Parser a c A → D ⊢ Parser a c B → D ⊢ Parser a c (A ⊕Set B)
  (p <|> q) = mkP λ K →
    ▷dec-map ⊗⊕-distL⁻ ⊗⊕-distL ∘⊢ ▷dec-⊕& ∘⊢ (pAt p K ,& pAt q K)

  -- Parser for each literal
  tok : (c : Alphabet) → D ⊢ Parser true false (litSet c)
  tok c = mkP λ K → ▷□ (dec-lit⊗-at c) ∘⊢ π₂

  -- Parser for any literal
  anyTok : D ⊢ Parser true false charSet
  anyTok = mkP λ K → ▷□ dec-char⊗-at ∘⊢ π₂

  -- Parser for ε
  nil : D ⊢ Parser false false εSet
  nil = mkP λ K → ▷dec-map ⊗ε-unit-l⁻ ⊗-unit-l ∘⊢ π₂

  -- Parser for ⊥
  fail : {a c : Bool} → D ⊢ Parser a c ⊥Set
  fail {c = c} = mkP λ K → ▷next {b = c} (dec-no ∘⊢ ⇒-intro (⊗⊥-annihL ∘⊢ π₂))

  -- a closed parser is available at every suffix
  box : {A : TheorySet ℓA tt} → ⊤Ty ⊢ Parser false false A
    → D ⊢ Parser true true A
  box p = mkP λ K → pApp K ∘⊢ (▷next {b = true} p ,&p id⊢)

□dec-ε : {D : TheoryTy ℓD tt} → D ⊢ ty (□ (DecSet ε↑Set))
□dec-ε = ▷next {b = false} (decLiftTheoryTy dec-ε)

-- A parser under the hypothesis ⊤ is sufficent for
-- buidling a decider for A
runP : {A : TheorySet ℓA tt} → ⊤Ty ⊢ Parser false false A → Decidable (ty A)
runP p =
...
module Fix {ℓA} (A : TheorySet ℓA tt) where
  ...
  call : ty (▷ (ParserSet false false A)) ⊢ Parser true true A
  call = mkP pApp

  -- Guarded fixpoints build closed parsers
  fix : ty (▷ (ParserSet false false A)) ⊢ Parser false false A
    → ⊤Ty ⊢ Parser false false A
  fix = löbG {A = ParserSet false false A}

  -- ...which are then used to build deciders
  decide : ty (▷ (ParserSet false false A)) ⊢ Parser false false A
    → Decidable (ty A)
  decide φ = runP (fix φ)
