...
-- The bridge to the next phase, not part of lexing: the parse tree already
-- *is* the tokenisation, and these only display it.

yield : ∀ {n} (r : RE n) → SemanticAction (ty ⟦ r ⟧) (List Alphabet)
yield εr = semact-pure []
yield ⊥r = ⊥Ty-elim
yield ⟨ c ⟩r = semact-pure (c ∷ [])
yield (satr P) = semact-map (_∷ []) semact-sat
yield (r ⊗r r') =
