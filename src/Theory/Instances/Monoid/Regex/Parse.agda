...
{- POSIX regex source text, elaborated to an `RE`.  The bootstrap, NOT
   verified: if it builds the wrong `RE`, the wrong language is decided,
   correctly.  A malformed pattern is a type error at the use site. -}
...
private
  ...
  -- Repetition needs a consuming body: `(a?)*`, the case every engine
  -- special-cases, is ruled out by type.
...
-- a malformed pattern is a type error: `IsJust nothing` is `⊥`, `Unit` solves by eta
