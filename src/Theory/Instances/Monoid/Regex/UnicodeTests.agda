...
{- POSIX classes on real text.

   Nothing here collapses to `Maybe`.  A match is a `ty ⟦ r ⟧ w` -- the
   parse itself -- and a non-match is a `¬Ty (ty ⟦ r ⟧) w`, a refutation of
   every parse.  That second one is the whole reason these are the
   *Decidable* combinators; `Maybe` would have discarded it. -}
open import Cubical.Foundations.Prelude
import Cubical.Data.Equality as Eq

module Theory.Instances.Monoid.Regex.UnicodeTests where
...
private
  ...
  -- the parse of `s` as an `r`, or a refutation of every parse
