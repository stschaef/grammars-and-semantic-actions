...
{- First sets, as refutations.

   `c ∉First A` says no `A`-word begins with `c`.  The one fact with content
   is `first⊗-split`: if `A` cannot be empty, then a `c`-prefix of `A ⊗ B`
   is a `c`-prefix of `A`.  The old proof went through `firstChar≅` and
   `same-first` over `A ⊗ string` (~30 lines); here it is one match on the
   left factor's word, which is the same argument `Precise` makes for
   `lit⊗-precise`.  It is the last element-level step in this layer. -}
...
open import Theory.Instances.Monoid.Residual Alphabet isSetAlphabet
  using (two-η ; &⊕-distR ; &⊕ᴰ-distR ; ⊗⊕ᴰ-distL)
...
-- The content: a `c`-prefix of `A ⊗ B`, with `A` non-nullable, is a
-- `c`-prefix of `A`.  Matching the left factor's word is what makes the
-- head letter available; `flat` reads it off the two splittings.
private
  ...
  split-go {A = A} {B = B} c nu m (d ∷ x) y e a b sw =
    ...
    where
    -- the two splittings agree on the head letter
    d≡c : d ≡ c
    d≡c = L.cons-inj₁
      (Eq.eqToPath e
...
-- Without non-nullability, split `A` first: either it is empty, in which
-- case the `c` is `B`'s, or it is not and the previous lemma applies.
∉First⊗ : {A : TheoryTy ℓA tt} {B : TheoryTy ℓB tt} {c : Alphabet}
  → c ∉First A → c ∉First B → c ∉First (A ⊗ B)
...
-- separation: no character can begin both
...
-- The star.  With `A` non-nullable, one unrolling is enough.
...
-- Without non-nullability the unrolling can loop, so the refutation is
-- built by a fold: at `nil` the empty word does not start with `c`, at
-- `cons` either the head is empty (and the tail's refutation applies) or
-- it is not (and `∉First⊗l` does).
