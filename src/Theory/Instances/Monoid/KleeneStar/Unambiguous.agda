...
{- `unambiguous (A *)` from `¬Nullable A`, `SeqUnambig A`, `unambiguous A`
   -- the hypotheses `*Aut` already demands. -}
...
-- named locally: `Theory/Type/Unambiguity/Base` reaches here by two paths
Unambig : TheoryTy ℓA tt → Type _
Unambig A = (m : String) → isProp (A m)

-- `_∉First_`/`_∉FollowLast_` from `SequentialUnambiguity`; the
-- combinatorial half (Levi + clash) is `Precise.splitAgree`.

SeqUnambig : TheoryTy ℓA tt → Type _
SeqUnambig A = (c : Alphabet) → (c ∉FollowLast A) Sum.⊎ (c ∉First A)

-- Both arguments matched as `roll m (b , payload)`; the tail descends
-- structurally (no `rec`, no pragma): `Automaton/Unambiguous`'s
-- `unambiguous-Trace` technique, with the `PathP`s built inline.
...
module Star* {A : TheoryTy ℓA tt}
  ...
  private
    ...
    -- the two decompositions cut in the same place: `c` would both follow
    -- a complete `A` and open the next piece
  ...
  unambig m (roll .m (true , ms , e , f)) (roll .m (false , ns , e' , f')) =
    nil-cons (ms zero) (ms (suc zero)) (f zero .lower)
      (Eq.eqToPath e ∙ sym (Eq.eqToPath e'))
  -- cons / cons.  Recursive call in the clause body: via `where` the
  -- checker compares the tail against the parameter and loses the descent.
