...
{- Regexes with capture groups and backreferences; captures are de Bruijn
   (`grpr` binds).  `satr` is dropped: its yield is undetermined, so it
   would not collapse the indexed continuation.  A backreference is
   nullable, which is what stops `(\1)*`. -}
...
-- only the publishing form; `toParser` recovers the ordinary one for free
