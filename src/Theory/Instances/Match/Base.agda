...
{- Values -- booleans and pairs -- as a theory.

   One sort and three operations, none of them indexed by external data,
   and no generators at all: `V` is `⊥`, so the free model is the *initial*
   algebra and every element of it is closed.  That is the point.  The
   annotated lambda instance has `V = ℕ` because a term may be a variable;
   a scrutinee may not, and a judgment that descends on an open term would
   have to say what to do at a generator.

   Two of the three operations are nullary.  A nullary operation has no
   slots, so a rule at one has nowhere to put a premise -- and nowhere to
   put a *refutation* either, which is what `Judgment`'s `clashAt` exists
   for: `Match ptrue` fails at `vfalse` for a reason that no slot can
   carry, so the failure is routed through `⊥Ty` and `Ans-map&` instead of
   through the node.  A signature all of whose operations have arity ≥ 1
   never notices the difference.

   The presentation is a bespoke `data Val` rather than the generic free
   model, for the usual reason: the tests are `refl`, so everything the
   checker touches has to reduce. -}
...
-- No generators.  Named, rather than written `λ ()` at each use, so that
-- every module downstream is instantiated at the *same* function and the
-- parameters match definitionally.
NoVar : Type ℓ-zero
NoVar = ⊥
...
isSetVSort : isSet VSort
isSetVSort = isOfHLevelRetract 2 (λ _ → tt) (λ _ → val) srtRet
  (Discrete→isSet λ _ _ → yes refl)
