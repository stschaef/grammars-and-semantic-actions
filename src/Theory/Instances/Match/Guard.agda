...
{- What the generic combinators ask of the value theory: precision, a
   well-founded order for the guard, and a cover by head constructor.
   `VOp` is FINITE, which is what makes `Exhaustive` possible.  Nullary slot
   tuples are unique by `funExt λ ()`; only the pair needs `pairFst`/`pairSnd`. -}
...
-- Argument positions of `vpairOp`; the nullary operations have none, which
-- is why they need `Judgment`'s `clashAt`.
pattern theFst = zero
