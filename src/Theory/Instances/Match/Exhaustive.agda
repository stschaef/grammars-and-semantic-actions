...
{- Exhaustive + irredundant = `Cover (Fin n) (λ i → Match (clause i))`
   (`total`/`disjoint`); `tally cs v ≡ 1` is its computational shadow.
   Not proved: a general "`Cover` implies `tally ≡ 1`" -- the enumeration
   is an ordered list, the cover is not. -}
...
-- the clause index is a retract of the operation
...
-- The off-diagonal cases are `Judgment`'s `clash` lemmas.
...
-- transfers the cover from `Fin 3` to the grammar the matcher runs on
...
-- The cover also decides each clause, never touching `fix`, `look`, or an
-- `AnswerFunctor`.
