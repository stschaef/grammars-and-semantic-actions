...
{- Bags with a CHOSEN head: the repair for `Bag/Failure`.  `Precise` is a property of the
   SIGNATURE, so the theory itself must change: `consOp x` carries its element in the
   operation and there are no equations, so `Precise` and no-confusion come back for free.
   Cost: the free model is `List El` — `Sorted`/`Occurs` decide properties of
   REPRESENTATIVES; `Quotient` says which descend along `toBag`.  `noVar` is named, not
   `λ ()` at each use: two absurd lambdas are not definitionally equal. -}
...
-- `op (consOp x) ms` is `x ∷ ms zero` definitionally: why the tests reduce
