...
{- What the combinators ask of the chosen-head theory, and everything
   `Bag/Failure` proves the commutative theory cannot supply.

   `preciseC` is the exact negation of `Failure`'s `¬preciseNode`, and the
   comparison is the point of both modules.  There, `{true,false}` was a
   node of `_⊙_` in two ways and the two ways disagreed in slot zero.
   Here `consOp x` fixes the head IN THE OPERATION, so a decomposition of
   `m` along it is a decomposition along a constructor, and the only thing
   left to determine is the tail -- read off by `tlOf`, a projection rather
   than a match, because the element occurs both as the operation's index
   and as a constructor argument and `Eq.refl` on `op (consOp x) ms ≡ x ∷ l`
   is stuck without K.

   `nodeCover` is the same contrast at the level of the cover.  `Failure`'s
   `¬disjointHead` refutes exactly this cover for bags: `{true,false}` has
   two heads.  For lists a head is a constructor, `clsC` reads it off, and
   `disjoint` is no-confusion.  `COp` is infinite -- `consOp` carries an
   element -- and, as in `Layout/Guard` and `Annotated/Guard`, a `Cover`
   does not care; what would care is a sum over the cells, and `Ans-map&`
   conditions on the cell instead.

   The order is the proper suffix, which for a one-argument operation is
   what `ilexOrder` on the term size degenerates to.  A recursive call is
   entitled to the tail and to nothing else, which is also the reason no
   judgment below can permute anything. -}
...
-- The one argument position `consOp x` has.  `nilOp` has none, so its slot
-- function is an absurd pattern everywhere it appears.
...
-- The measure: how many elements are left.
cSize : List El → ℕ
cSize [] = 0
...
-- The node cover: by head constructor, which for this signature is "empty,
-- or else which element is in front".
clsC : List El → COp
clsC [] = nilOp
clsC (x ∷ _) = consOp x
