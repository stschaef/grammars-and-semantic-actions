...
{- Converse of `Correct`, over plain substitutions `Fin n → Tm m` (for
   which restriction along `thin x` IS composition): `Sol` ≃ `Solvable`. -}
...
-- `Thicked x y c` records what `c ≡ thick x y` says; no clause below
...
-- `check x u ≡ just w` says `w` is `u` with `x` thinned out.
...
-- Restriction: the step `Correct` could not take for a chain.
...
-- Clash: `flatSound` backwards.  Over `bind ρ`, not an abstract
-- fork-commuting `f`: only a substitution must send `leaf` to `leaf`.
private
...
-- Occurs, part one: `flatA` never pushes `x ≟ var x` (`sameF` discards it).
...
-- Occurs, part two: a failed check makes `ρ x` a proper subterm of
-- `bind ρ u`, given `u ≢ var x` from part one.
