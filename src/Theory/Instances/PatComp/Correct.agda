...
{- What the derivation does not carry, proved once.

   `Ok` is a structural invariant: it says the tree has the shape the
   algorithm produces, branch for branch.  The SEMANTIC claim -- running
   the tree is running the matrix -- is the theorem `sound`, and this file
   is the argument that the first implies the second.

   The whole of the content is in two lemmas, and they are Maranget's two:

     specRun   the rows that survive specialisation on `v`'s head, run
               against `v`'s arguments and the rest of the vector, decide
               exactly what the original matrix decides at `v`
     dfltRun   ...and when the head column does not mention `v`'s head at
               all, the default matrix does

   Both are inductions on the matrix with a case per (head pattern, head
   value) pair, and neither mentions trees.  Everything after them is
   bookkeeping: `sound` is an induction on the tree in which `Ok` supplies,
   at each switch, exactly the premise the lemma needs.

   EXHAUSTIVENESS AND REDUNDANCY THEN FALL OUT, and they fall out as
   properties of the TREE rather than as separate analyses.

     * `tfail` is emitted only at an empty matrix, so a tree with no
       `tfail` in it is a matrix that matches everything: `exhaustive`.
       This is the deliverable `Match/Exhaustive` could only state one
       clause list at a time -- there a complete irredundant list WAS a
       `Cover`, exhibited by hand; here a syntactic test on the compiler's
       output decides it for every matrix.
     * `runTree` only ever returns a label that occurs in the tree, so a
       row whose index labels no leaf is never selected: `redundant`.

   The converse of the second -- every label in the tree is reachable --
   is NOT proved, and it is the honest gap.  It is true of this algorithm,
   but a proof has to construct a value vector reaching a given leaf, which
   is a different induction (on the tree, building the vector) than any
   here.  What is proved is the direction a compiler warning needs: a row
   the tree never names is dead. -}
...
-- Maranget's two lemmas.  Sixteen clauses each, one per (head pattern,
-- head value) pair plus the empty matrix, and the only one that is not a
-- congruence is the pair against a pair -- where the two columns the
-- constructor was expanded into have to be reassociated back.
...
-- Reading `Ok` at a switch: which of the two subtrees `pick` will consult,
-- and what the derivation says about it.  The dispatch is on the TREE, so
-- the boolean `heads P o` is never matched on -- it is recovered from the
-- branch's shape instead, which is the direction that reduces.
private
  ...
  -- a constructor missing from the head column is a head column that is
  -- not a complete signature, one `and` at a time
...
-- The switch step, uniform in the head value: `peel` and `clsV` are what
-- make one lemma serve all three constructors.
private
...
-- ...and the specification, as a grammar the front end can return.
Agrees : (n : ℕ) → Mat n → Type ℓ-zero
Agrees n P = Σ[ t ∈ Tree ] ((vs : Vals n) → runTree t vs ≡ matrixRun P vs)


-- EXHAUSTIVENESS.  A decidable test on the compiler's output.
noFail : Tree → Bool
noFail tfail = false
noFail tskip = true
noFail (tleaf _) = true
...
-- REDUNDANCY.  A row the tree never names is a row no value selects.
labels : Tree → List ℕ
labels tfail = []
labels tskip = []
labels (tleaf k) = k ∷ []
