-- Disjointness; `disjoint A B` is the binary case of a `Cover`'s `disjoint`.
...
isUnambiguousRetract r =
  unambiguousRetract (r .weak .fun) (r .weak .inv) (r .ret)

unambiguous≅ : A ≅ B → unambiguous A → unambiguous B
unambiguous≅ e = unambiguousRetract (e .inv) (e .fun) (e .sec)
...
unambiguousRetract'→≅ f g r uB =
  unambiguous→≅ (unambiguousRetract f g r uB) uB f g
