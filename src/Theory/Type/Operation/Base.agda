...
-- `⊗-elim`'s computing sibling: projects the splitting instead of matching it,
-- so it does not block on quotient models.  (Bags/Quicksort/Tests: 3.3s vs >7min.)
...
-- with a value alongside: how a delayed hypothesis reaches the slots
⊗&-overSplit : {o : σ .ops} {ℓs ℓs' : arities σ o → Level}
  {As : Args (σ .arity o) ℓs (σ .sortOf o)}
  {As' : Args (σ .arity o) ℓs' (σ .sortOf o)}
