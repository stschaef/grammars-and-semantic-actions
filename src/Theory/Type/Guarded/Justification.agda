...
open import Cubical.Relation.Nullary.Base using (Dec ; Discrete)
...
module _ {X : Type ℓX} {xs : X → S} where
  ...
  -- the *only* place a consumer's size argument has to go
  ...
  -- re-exported so a consumer names it without naming an order
  ...
  -- lexicographic (input size, index rank): shrink the input, or keep size and drop rank
  ...
  -- The same vocabulary against an order `W` on inputs rather than their size.
  ...
  -- Löb by structural recursion on a fuel; reduces on canonical input.
