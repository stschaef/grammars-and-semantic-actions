...
-- `A ⊢ Ω`, and `subobject p` is its pullback of `true`.
...
module Theory.Type.Subobject.Base
...
-- `p ≡ true ∘⊢ ⊤Ty-intro` internalises "`p` holds of every element of `A`"
module Subobject {ℓ'} {s : S} {A : TheoryTy ℓA s} (p : A ⊢ Ω {ℓ' = ℓ'}) where
  subobject : TheoryTy (ℓ-max ℓA ℓ') s
  subobject m = Σ[ x ∈ A m ] ⟨ p m x ⟩
  ...
  module _ {B : TheoryTy ℓB s} (f : B ⊢ subobject) where
    private
      the-path : p ∘⊢ sub-π ∘⊢ f ≡ true ∘⊢ ⊤Ty-intro
      the-path = cong (_∘⊢ f) sub-π-pf

    -- η on the nose: the second component is a proposition
    sub-η : f ≡ sub-intro (sub-π ∘⊢ f) the-path
    sub-η i = f

  subobject-section : (f : A ⊢ subobject) → sub-π ∘⊢ f ≡ id⊢
    → p ≡ true ∘⊢ ⊤Ty-intro
  subobject-section f sec =
...
module _ {ℓ'} {X : Type ℓX} {xs : X → S}
  ...
  subobject-ind-alg : ∀ x
    → ⟦ F x ⟧TheoryTy (λ y → subobject (p y)) ⊢ subobject (p x)
  subobject-ind-alg x =
    sub-intro (p x) (roll ∘⊢ map (F x) (λ y → sub-π (p y))) (pf x)

  subobject-ind' : ∀ (x : X) → μ F x ⊢ subobject (p x)
  subobject-ind' = rec F subobject-ind-alg

  -- `sub-π` is an algebra map by `sub-β`, so `rec-section` does the induction
  sub-π-section : ∀ x → sub-π (p x) ∘⊢ subobject-ind' x ≡ id⊢
  sub-π-section = rec-section F subobject-ind-alg (λ y → sub-π (p y)) λ y → refl

  subobject-ind : ∀ (x : X) → p x ≡ true ∘⊢ ⊤Ty-intro
  subobject-ind x =
    subobject-section (p x) (subobject-ind' x) (sub-π-section x)

module _ {s : S} {A : TheoryTy ℓA s} {B : TheoryTy ℓB s}
  (f : B ⊢ A) (p : A ⊢ Ω {ℓ' = ℓ'}) where

  preimage : TheoryTy (ℓ-max ℓB ℓ') s
  preimage = subobject (p ∘⊢ f)

  preimage-map : preimage ⊢ subobject p
...
module _ {s : S} {A : TheoryTy ℓA s} {B : TheoryTy ℓB s}
  ...
  mono→subobject : TheoryTy (ℓ-max ℓA ℓB) s
  mono→subobject = subobject mono-prop
...
module _ {s : S} {A : TheoryTy ℓA s} (unambig-A : unambiguous A)
  ...
  unambiguous→subobject : TheoryTy (ℓ-max ℓB ℓA) s
  unambiguous→subobject = subobject unambiguous-prop
